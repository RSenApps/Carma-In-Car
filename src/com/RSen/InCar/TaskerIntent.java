...
public class TaskerIntent extends Intent {
	...
	public final static String TASKER_PACKAGE_CUPCAKE = TASKER_PACKAGE
			+ "cupcake";

	// Play Store download URLs
	public final static String MARKET_DOWNLOAD_URL_PREFIX = "market://details?id=";
	private final static String TASKER_MARKET_URL = MARKET_DOWNLOAD_URL_PREFIX
			+ TASKER_PACKAGE_MARKET;
	private final static String TASKER_MARKET_URL_CUPCAKE = MARKET_DOWNLOAD_URL_PREFIX
			+ TASKER_PACKAGE_CUPCAKE;
	...
	public final static String ACTION_TASK_COMPLETE = TASKER_PACKAGE
			+ ".ACTION_TASK_COMPLETE";
	public final static String ACTION_TASK_SELECT = TASKER_PACKAGE
			+ ".ACTION_TASK_SELECT";
	...
	// Misc
	private final static String PERMISSION_RUN_TASKS = TASKER_PACKAGE
			+ ".PERMISSION_RUN_TASKS";

	private final static String ACTION_OPEN_PREFS = TASKER_PACKAGE
			+ ".ACTION_OPEN_PREFS";
	...
	private final static String TASKER_PREFS_URI = "content://"
			+ TASKER_PACKAGE + "/prefs";
	...
	public static String getInstalledTaskerPackage(Context context) {
		...
		try {
			context.getPackageManager()
					.getPackageInfo(TASKER_PACKAGE_MARKET, 0);
	...
	public static Intent getTaskerInstallIntent(boolean marketFlag) {
		...
		return new Intent(
				Intent.ACTION_VIEW,
				Uri.parse(marketFlag ? ((SDKVersion() == CUPCAKE_SDK_VERSION) ? TASKER_MARKET_URL_CUPCAKE
						: TASKER_MARKET_URL)
						: TASKER_DOWNLOAD_URL));
	}
	...
	public static IntentFilter getCompletionFilter(String taskName) {
		...
		IntentFilter filter = new IntentFilter(
				TaskerIntent.ACTION_TASK_COMPLETE);
	...
	public static boolean havePermission(Context c) {
		return c.checkPermission(PERMISSION_RUN_TASKS, Process.myPid(),
	...
	public static Intent getExternalAccessPrefsIntent() {
		return new Intent(ACTION_OPEN_PREFS).putExtra(EXTRA_OPEN_PREFS_TAB_NO,
				MISC_PREFS_TAB_NO);
	}
	...
	public TaskerIntent setTaskPriority(int priority) {
		...
		if (validatePriority(priority))
			...
			Log.e(TAG, "priority out of range: " + MIN_PRIORITY + ":"
					+ MAX_PRIORITY);
	...
	public TaskerIntent addAction(int code) {
		...
		putExtra(EXTRA_ACTION_INDEX_PREFIX + Integer.toString(actionCount),
				actionBundle);
	...
	public TaskerIntent addArg(String pkg, String cls) {
		...
		if (b != null) {
			...
			b.putString(ARG_INDEX_PREFIX + Integer.toString(argCount++),
					b.toString());
		}
	...
	public boolean receiverExists(Context context) {
		List<ResolveInfo> recs = context.getPackageManager()
				.queryBroadcastReceivers(this, 0);
	...
	private Bundle getActionBundle() {
		...
		if (argCount > MAX_NO_ARGS)
			...
			String key = EXTRA_ACTION_INDEX_PREFIX
	...
	private static boolean prefSet(Context context, String col) {
		...
		Cursor c = context.getContentResolver().query(
				Uri.parse(TASKER_PREFS_URI), proj, null, null, null);
