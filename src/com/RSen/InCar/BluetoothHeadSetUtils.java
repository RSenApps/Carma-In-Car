...
public abstract class BluetoothHeadSetUtils {
	...
	private boolean startBluetooth() {
		...
		if (mBluetoothAdapter != null) {
			if (mAudioManager.isBluetoothScoAvailableOffCall()) {
				...
				// Need to set audio mode to MODE_IN_CALL for call to
				// startBluetoothSco() to succeed.
				mAudioManager.setMode(AudioManager.MODE_IN_CALL);

				mIsCountDownOn = true;
				// mCountDown repeatedly tries to start bluetooth Sco audio
				// connection.
				mCountDown.start();
	...
	private boolean startBluetooth11() {
		BluetoothProfile.ServiceListener mHeadsetProfileListener = new BluetoothProfile.ServiceListener() {
			...
			public void onServiceConnected(int profile, BluetoothProfile proxy) {
				...
				// During the active life time of the app, a user may turn on
				// and off the headset.
				...
				// Calling startVoiceRecognition does not result in immediate
				// audio connection.
	...
	private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(BluetoothDevice.ACTION_ACL_CONNECTED)) {
				...
				// override this if you want to do other thing when the device
				// is disconnected.
				...
				if (state == AudioManager.SCO_AUDIO_STATE_CONNECTED) {
					...
					if (mIsStarting) {
						// When the device is connected before the application
						// starts,
						// ACTION_ACL_CONNECTED will not be received, so call
					...
					// override this if you want to do other thing when Sco
					// audio is connected.
					onScoAudioConnected();
					...
					// Always receive SCO_AUDIO_STATE_DISCONNECTED on call to
					// startBluetooth()
					// which at that stage we do not want to do anything. Thus
					// the if condition.
					if (!mIsStarting) {
	...
	private CountDownTimer mCountDown = new CountDownTimer(10000, 1000) {
		...
		public void onTick(long millisUntilFinished) {
			// When this call is successful, this count down timer will be
			// canceled.
